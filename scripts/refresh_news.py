...
'''
    ...
    ri = run_info or {}
    if ri:
        total_fresh = ri.get('fresh', 0)
    else:
        total_fresh = 0
    ri_time = ri.get('time', '?') if ri else '?'
    ri_cached = ri.get('cached', 0) if ri else 0
    refresh_html = f'<div class="refresh-info">🕐 最后更新: {ri_time} | 📥 本次爬取 {total_fresh} 篇 | 📚 页面展示 {total} 篇 | 💾 缓存 {ri_cached} 篇</div>' if ri else ''
